...
def get_round() -> str:
    ...
    screen_capture = ImageGrab.grab(
        bbox=screen_coords.ROUND_POS_ONE.get_coords())
    round_two_x = screen_capture.crop(screen_coords.ROUND_POS_TWO.get_coords())
    game_round = ocr.get_text_from_image(
        image=round_two_x, whitelist=ocr.ROUND_WHITELIST)
    if game_round in game_assets.ROUNDS:
        return game_round

    round_one_x = screen_capture.crop(
        screen_coords.ROUND_POS_ONE.get_coords())
    game_round = ocr.get_text_from_image(
        image=round_one_x, whitelist=ocr.ROUND_WHITELIST)
    return game_round
